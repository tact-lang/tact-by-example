var w=Object.defineProperty;var m=(n,e,t)=>e in n?w(n,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):n[e]=t;var c=(n,e,t)=>(m(n,typeof e!="symbol"?e+"":e,t),t);import{S as I,i as C,s as y,I as p,ac as B}from"../../../../chunks/index-1d4083c1.js";import{d as r,g as l,s as g}from"../../../../chunks/helpers-e86371c8.js";import{d as f}from"../../../../chunks/index-7e0e6ef9.js";const D=`# Constants

Unlike variables, constants cannot change. Their values are calculated in *compile-time* and cannot change during execution.

Constant initializations must be relatively simple and only rely on values known during compilation. If you add two numbers for example, the compiler will calculate the result during build and put the result in your compiled code.

You can read constants both in *receivers* and in *getters*.

Unlike contract variables, constants don't consume space in persistent state. Their values are stored directly in the code cell.`,E=`import "@stdlib/deploy";

// global constants are calculated in compile-time and can't change
const GLOBAL_CONST1: Int = 1000 + ton("1.24") + pow(10, 9);

contract Constants with Deployable {

    // contract constants are calculated in compile-time and can't change
    const CONTRACT_CONST1: Int = 2000 + ton("1.25") + pow(10, 9);
    
    // if your contract can be in multiple states, constants are an easy alternative to enums
    const STATE_UNPAID: Int = 0;
    const STATE_PAID: Int = 1;
    const STATE_DELIVERED: Int = 2;
    const STATE_DISPUTED: Int = 3;

    init() {}

    get fun sum(): Int {
        // you can read the constants anywhere
        return GLOBAL_CONST1 + self.CONTRACT_CONST1 + self.STATE_PAID;
    }
}`;function J(n){return e=>{let t=e;t.storeUint(2490013878,32),t.storeUint(n.queryId,64)}}async function d(){const n=r.Cell.fromBase64(""),e=r.Cell.fromBase64("");let t=r.beginCell();t.storeRef(e),t.storeUint(0,1);const o=t.endCell();return{code:n,data:o}}const b={2:{message:"Stack undeflow"},3:{message:"Stack overflow"},4:{message:"Integer overflow"},5:{message:"Integer out of expected range"},6:{message:"Invalid opcode"},7:{message:"Type check error"},8:{message:"Cell overflow"},9:{message:"Cell underflow"},10:{message:"Dictionary error"},13:{message:"Out of gas error"},32:{message:"Method ID not found"},34:{message:"Action is invalid or not supported"},37:{message:"Not enough TON"},38:{message:"Not enough extra-currencies"},128:{message:"Null reference exception"},129:{message:"Invalid serialization prefix"},130:{message:"Invalid incoming message"},131:{message:"Constraints error"},132:{message:"Access denied"},133:{message:"Contract stopped"},134:{message:"Invalid argument"},135:{message:"Code of a contract was not found"},136:{message:"Invalid address"},137:{message:"Masterchain support is not enabled for this contract"}};class i{constructor(e,t){c(this,"address");c(this,"init");c(this,"abi",{errors:b});this.address=e,this.init=t}static async init(){return await d()}static async fromInit(){const e=await d(),t=r.contractAddress(0,e);return new i(t,e)}static fromAddress(e){return new i(e)}async send(e,t,o,s){let a=null;if(s&&typeof s=="object"&&!(s instanceof r.Slice)&&s.$$type==="Deploy"&&(a=r.beginCell().store(J(s)).endCell()),a===null)throw new Error("Invalid message type");await e.internal(t,{...o,body:a})}async getSum(e){let t=new r.TupleBuilder;return(await e.get("sum",t.build())).stack.readBigNumber()}}function v(n,e,t){let o;p(n,g,a=>t(2,o=a));let s;return B(g,o={markdown:D,tactCode:E,deploy:async()=>{const a=await f.Blockchain.create(),A=await a.treasury("deployer");A.getSender(),s=a.openContract(await i.fromInit());const u={[A.address.toString()]:"deployer",[s.address.toString()]:"contract"};return[s,u,[await s.send(A.getSender(),{value:r.toNano(1)},{$$type:"Deploy",queryId:0n})]]},messages:{},getters:{sum:async()=>await s.getSum()},prev:l(import.meta.url).prev,next:l(import.meta.url).next},o),[]}class M extends I{constructor(e){super(),C(this,e,v,null,y,{})}}export{M as default};
