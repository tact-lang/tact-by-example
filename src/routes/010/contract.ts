...
async function Counter_init(owner: Address) {
  ...
  /* let system = await ContractSystem.create();
    ...
    if (res.exitCode !== 0 && res.exitCode !== 1) {
        if (Counter_errors[res.exitCode]) {
            throw new ComputeError(Counter_errors[res.exitCode].message, res.exitCode, { logs: res.logs });
        } else {
  ...
  let executor = SmartContract.create(blockchain, {
