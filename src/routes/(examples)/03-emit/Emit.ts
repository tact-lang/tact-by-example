...
export function loadStakeEvent(slice: Slice) {
  let sc_0 = slice;
  if (sc_0.loadUint(32) !== 2917934626) {
    throw Error("Invalid prefix");
  }
  let _amount = sc_0.loadCoins();
...
export class Emit implements Contract {
  ...
  readonly abi: ContractABI = {
    types: [
      ...
      { name: "TransferEvent", header: 772744475, fields: [] },
      { name: "StakeEvent", header: 2917934626, fields: [] },
    ],
    errors: Emit_errors,
  };
  ...
  async send(
    ...
    message: "action" | "transfer" | "stake" | Deploy,
  ) {
