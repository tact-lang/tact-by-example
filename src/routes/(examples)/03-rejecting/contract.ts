...
type Rejecting_init_args = {
  $$type: "Rejecting_init_args";
};

function initRejecting_init_args(src: Rejecting_init_args) {
  return (builder: Builder) => {
    let b_0 = builder;
  };
}

async function Rejecting_init() {
...
export class Rejecting implements Contract {
  static async init() {
    return await Rejecting_init();
  }

  static async fromInit() {
    const init = await Rejecting_init();
    const address = contractAddress(0, init);
    return new Rejecting(address, init);
  }

  static fromAddress(address: Address) {
    return new Rejecting(address);
  }
