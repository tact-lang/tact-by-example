<script lang="ts">
  ...
  $store = {
    ...
    messages: {
      "increment (from deployer)": async () => {
        return [await contract.send(sender, { value: toNano(1) }, "increment")];
      },
      "increment (from sender 2)": async () => {
        return [await contract.send(sender2, { value: toNano(1) }, "increment")];
      },
      "double (from deployer)": async () => {
        return [await contract.send(sender, { value: toNano(1) }, "double")];
      },
      "double (from sender 2)": async () => {
        return [await contract.send(sender2, { value: toNano(1) }, "double")];
      },
    },
    getters: {
      owner: async () => {
