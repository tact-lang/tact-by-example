<script lang="ts">
  ...
  import { VariablesConsts } from "./contract";

  let sender: Sender;
  let contract: SandboxContract<VariablesConsts>;
  ...
  $store = {
    ...
    deploy: async () => {
      ...
      contract = blockchain.openContract(await VariablesConsts.fromInit(17n));
    ...
    getters: {
      sum: async () => {
        return await contract.getSum(18n);
      },
    },
